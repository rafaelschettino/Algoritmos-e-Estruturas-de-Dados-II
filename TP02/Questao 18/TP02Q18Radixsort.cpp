...
void radixsort(){
	int i;
	Personagem maior = clone(array[0]);
	Personagem ordenado[n];
	int exp = 1;

	for(i = 0; i < n; i++){
		if(array[i].altura > maior.altura){
			maior = clone(array[i]);
		}//fim if
	}//fim for i
	
	while((maior.altura / exp) > 0){
		int bucket[10] = {0};
		
		for(i = 0; i < n; i++){
			bucket[(array[i].altura / exp) % 10]++;
		}//fim for i
		...
		for(i = n-1; i >= 0; i--){
			ordenado[bucket[(array[i].altura/exp) % 10] - 1] = clone(array[i]);
			bucket[(array[i].altura/exp) % 10]--;
		}//fim for i

		for(i = 0; i < n; i++){
			array[i] = clone(ordenado[i]);
		}//fim for i
		exp *= 10;
	}//fim while
