...
/**
*Classe TP02Q07PesquisaSequencial
*@author Rafael Schettino
*@version mar/2020
*/
public class TP02Q07PesquisaSequencial{
	public static void main(String[] args) throws Exception{
		...
		for(String nome_pesquisa = MyIO.readLine(); nome_pesquisa.equals("FIM") == false; nome_pesquisa = MyIO.readLine()){
			boolean resp = lista.pesquisar(nome_pesquisa);
			if(resp){
				MyIO.println("SIM");
			}
			else{
		...
		escreverArq.printf("651636"+"\t"+time+"\t"+lista.getContador());
...
class Lista{
    ...
    private int n;
    private int contador = 0; //Conta o numero de comparacoes realizadas pelo programa
   ...
   public boolean pesquisar(String x) {
    ...
    for(int i = 0; i < n && resp == false; i++){
        if(array[i].getNome().compareTo(x) == 0){
            resp = true;
        }//fim if
        contador++;
    }//fim for i
